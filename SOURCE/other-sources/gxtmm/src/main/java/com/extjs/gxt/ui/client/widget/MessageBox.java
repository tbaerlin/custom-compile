...
public class MessageBox {
    ...
    /**
     * Handler that monitors keyboard presses and changes the focus accordingly
     */
    private static KeyDownHandler keyboardHandler = new KeyDownHandler() {
        @Override
        public void onKeyDown(KeyDownEvent keyDownEvent) {
            if (keyDownEvent.isLeftArrow() || keyDownEvent.isRightArrow()) {
                com.google.gwt.dom.client.Element focusedElement = getFocusedElement();

                if (isInput(focusedElement)) {
                    return;
                }

                keyDownEvent.preventDefault();

                Dialog dialog = (Dialog) keyDownEvent.getSource();
                ButtonBar btnBar = dialog.getButtonBar();

                int focusPosition = findFocusPosition(focusedElement, btnBar);
                int newFocusPosition = calculateNewFocusPosition(keyDownEvent, btnBar, focusPosition);
                btnBar.getItem(newFocusPosition).focus();
            }
        }

        private int calculateNewFocusPosition(KeyDownEvent keyDownEvent, ButtonBar btnBar,
                int oldFocusPosition) {
            if (keyDownEvent.isLeftArrow()) {
                if (oldFocusPosition > 0) {
                    return oldFocusPosition - 1;
                }
                else if (oldFocusPosition == -1) {
                    return btnBar.getItemCount() - 1;
                }
            }
            else if (keyDownEvent.isRightArrow()) {
                if (oldFocusPosition < btnBar.getItemCount() - 1) {
                    return oldFocusPosition + 1;
                }
            }
        ...
        private int findFocusPosition(com.google.gwt.dom.client.Element focusedElement,
                ButtonBar btnBar) {
            int position = -1;
            for (int i = 0; i < btnBar.getItems().size(); i++) {
                El realBtn = btnBar.getItem(i).el().selectNode("button"); // $NON-NLS-0$
                if (realBtn.dom == focusedElement) {
                    position = i;
                    break;
                }
            }
        ...
        public native com.google.gwt.dom.client.Element getFocusedElement() /*-{
            return $doc.activeElement;
        }-*/;

        public native boolean isInput(com.google.gwt.dom.client.Element element) /*-{
    ...
    public void addKeyboardHandler() {
        getDialog().addDomHandler(keyboardHandler, KeyDownEvent.getType());
    }
}
