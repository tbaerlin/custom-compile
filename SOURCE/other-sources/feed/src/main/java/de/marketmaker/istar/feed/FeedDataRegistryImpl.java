...
/**
 * an Implementation of FeedDataRegistry using AsciiString and ConcurrentHashMap
 * instead of ByteString and synchronized HashMap
 *
 * trying to reduce GC pressure since AsciiString can reuse existing byte arrays, we don't
 * need to copy and we can loose the synchronized
 */
@ManagedResource
public class FeedDataRegistryImpl implements FeedDataRegistry {

    private final Logger logger = LoggerFactory.getLogger(getClass());

    private final ConcurrentHashMap<AsciiString, FeedData> repository;
    ...
    public FeedDataRegistryImpl() {
        this(100);
    }

    public FeedDataRegistryImpl(int expectedSize) {
        this(new FeedMarketRepository(true), expectedSize);
    }

    public FeedDataRegistryImpl(FeedMarketRepository marketRepository, int expectedSize) {
        this.marketRepository = marketRepository;
        this.repository = new ConcurrentHashMap<>(expectedSize * 4 / 3 + 1);
    }
    ...
    public int removeMarket(String name) {
        ...
        for (FeedData fd : market.getElements(false)) {
            ByteString bs = fd.getVwdcode();
            if (remove(new AsciiString(bs.getBytes(), false)) != null) {
                n++;
            }
        }
    ...
    private FeedData remove(AsciiString bs) {
    ...
    protected FeedData unregister(ByteString key) {
        ...
        if (this.marketRepository.isWithMarketElements()) {
            ...
            fd = remove(new AsciiString(key.getBytes(), false));
        }
        if (fd != null) {
            ackRemove(fd);
        }
    ...
    private void register(FeedData data, boolean doNotifyListeners) {
        final AsciiString key = new AsciiString(data.getVwdcode().toString());
        final FeedData existing = this.repository.putIfAbsent(key, data);
        if (existing != null) {
            this.logger.warn("<register> existing feedDate " + toString(existing));
        }
        else {
    ...
    public
    @Nullable
    FeedData get(AsciiString asciiString) {
        return repository.get(asciiString);
    }
    ...
    public FeedData get(ByteString key) {
        return get(new AsciiString(key.getBytes(), false));
    }
    ...
    private void delete(FeedData fd) {
        final FeedData removed = remove(new AsciiString(fd.getVwdcode().getBytes(), false));
    ...
    private String toString(FeedData fd) {
        return (fd == null)
                ? "null"
                : fd.getVendorkey() + "/" + fd.getVendorkeyType();
    }
}
