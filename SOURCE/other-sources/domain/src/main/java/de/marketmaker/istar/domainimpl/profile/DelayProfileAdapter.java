 ...
 */
public class DelayProfileAdapter implements ProfileAdapter, Serializable {
    private static final long serialVersionUID = 1L;

    private class DelayProfileAdapterProfile implements Profile, Serializable {
        private static final long serialVersionUID = 1L;

        private Profile delegate;

        private DelayProfileAdapterProfile(Profile delegate) {
            this.delegate = delegate;
        }

        public String getName() {
            return "Delayed " + this.delegate.getName();
        }
        ...
        private PriceQuality adapt(PriceQuality result) {
            return result == PriceQuality.REALTIME ? PriceQuality.DELAYED : result;
        }
        ...
        public Profile toAspectSpecificProfile(Aspect aspect) {
            ...
            return (aspect != Aspect.PRICE)
                    ? result
                    : DelayProfileAdapter.this.adapt(result);
        }
    }

    public Profile adapt(Profile p) {
        return new DelayProfileAdapterProfile(p);
    }
}
