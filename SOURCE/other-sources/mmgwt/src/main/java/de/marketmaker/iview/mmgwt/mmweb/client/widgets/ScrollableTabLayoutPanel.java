/*
 * ScrollableTabLayoutPanel.java
 *
 * Created on 08.09.2015 15:15
 *
 * Copyright 2009 Google Inc.
 * Copyright 2015 vwd GmbH. All Rights Reserved.
 *
...
/**
 * A TabLayoutPanel with a scrollable tab bar.
 * Mostly is copy of TabLayoutPanel, except for the tab bar, which is wrapped by a FloatingPanel and except for
 * the layout, where it prefers a DockLayoutPanel to a LayoutPanel.
 * @see com.google.gwt.user.client.ui.TabLayoutPanel
 * @see FloatingPanel
 */
@SuppressWarnings("unused")
public class ScrollableTabLayoutPanel extends ResizeComposite implements HasWidgets,
    ...
    private class Tab extends SimplePanel {
        ...
        public Tab(Widget child) {
            ...
            this.inner.setClassName(TAB_INNER_STYLE);
        ...
        public boolean remove(Widget w) {
            ...
            if (replacingWidget || index < 0) {
                ...
                return ScrollableTabLayoutPanel.this.remove(index);
            }
        }

        public void setSelected(boolean selected) {
            if (selected) {
                addStyleDependentName("selected"); // $NON-NLS$
            } else {
                removeStyleDependentName("selected"); // $NON-NLS$
            }
        }
    ...
    private class TabbedDeckLayoutPanel extends DeckLayoutPanel {
        ...
        public void add(Widget w) {
            throw new UnsupportedOperationException("Use ScrollableTabLayoutPanel.add() to alter the DeckLayoutPanel"); // $NON-NLS$
        }

        @Override
        public void clear() {
            throw new UnsupportedOperationException("Use ScrollableTabLayoutPanel.clear() to alter the DeckLayoutPanel"); // $NON-NLS$
        }

        @Override
        public void insert(Widget w, int beforeIndex) {
            throw new UnsupportedOperationException("Use ScrollableTabLayoutPanel.insert() to alter the DeckLayoutPanel"); // $NON-NLS$
        }
    ...
    private static final String CONTENT_CONTAINER_STYLE = "gwt-TabLayoutPanelContentContainer"; // $NON-NLS$
    private static final String CONTENT_STYLE = "gwt-TabLayoutPanelContent"; // $NON-NLS$
    private static final String TAB_STYLE = "gwt-TabLayoutPanelTab";  // $NON-NLS$

    private static final String TAB_INNER_STYLE = "gwt-TabLayoutPanelTabInner"; // $NON-NLS$

    private final TabbedDeckLayoutPanel deckPanel = new TabbedDeckLayoutPanel();
    private final FlowPanel tabBar = new FlowPanel();
    private final FloatingPanel floatingPanel;
    ...
    public ScrollableTabLayoutPanel(double barHeight, Style.Unit barUnit) {
        DockLayoutPanel panel = new DockLayoutPanel(barUnit);
        initWidget(panel);

        this.tabBar.getElement().getStyle().setWhiteSpace(Style.WhiteSpace.NOWRAP);

        // Add the tab bar to the panel.
        this.floatingPanel = new FloatingPanel(FloatingPanel.Orientation.HORIZONTAL, true).withWidget(this.tabBar);
        panel.addNorth(this.floatingPanel, barHeight);
        ...
        setStyleName("gwt-TabLayoutPanel");  // $NON-NLS$
    }
    ...
    private void checkChild(Widget child) {
        assert getWidgetIndex(child) >= 0 : "Child is not a part of this panel"; // $NON-NLS$
    }

    private void checkIndex(int index) {
        assert (index >= 0) && (index < getWidgetCount()) : "Index out of bounds"; // $NON-NLS$
    }

    private void insert(final Widget child, Tab tab, int beforeIndex) {
        assert (beforeIndex >= 0) && (beforeIndex <= getWidgetCount()) : "beforeIndex out of bounds"; // $NON-NLS$
        ...
        this.floatingPanel.onResize();
        this.floatingPanel.scrollTo(tab.getElement());
    }
}
