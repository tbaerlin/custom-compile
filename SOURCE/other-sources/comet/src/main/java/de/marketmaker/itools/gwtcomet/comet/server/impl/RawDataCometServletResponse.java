...
import de.marketmaker.itools.gwtcomet.comet.client.impl.HTTPRequestCometTransport;
...
public abstract class RawDataCometServletResponse extends ManagedStreamCometServletResponseImpl {
    ...
    final static Logger logger = LoggerFactory.getLogger(RawDataCometServletResponse.class);
	...
	protected void doHeartbeat() throws IOException {
		appendMessageHeader();
		writer.append('#');
        logger.trace(toString() + " is performing heartbeat! / thread: [" + Thread.currentThread().getName() + "]");
        appendMessageTrailer();
	}
