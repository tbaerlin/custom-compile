...
public class OperaEventSourceCometTransport extends RawDataCometTransport {
	...
	public void connect(int connectionCount) {
        ...
        eventSource.setAttribute("src", getUrl(connectionCount));
	}
	
	@Override
	public void disconnect() {
		if (eventSource != null) {
            eventSource.setAttribute("src", "");
			eventSource = null;
			connected = false;
		}
	}
	...
	private native Element createEventSource(OperaEventSourceCometTransport client) /*-{
		...
		var eventHandler = $entry(function(event) {
			client.@de.marketmaker.itools.gwtcomet.comet.client.impl.OperaEventSourceCometTransport::onEvent(Ljava/lang/String;)(event.data);
		});
